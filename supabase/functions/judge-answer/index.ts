...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (body.mode === 'tutor-chat') {
      return await handleTutorChat(body, corsHeaders);
    }

    const { question, userAnswer, rubric } = body;
...
async function handleTutorChat(body: Record<string, unknown>, corsHeaders: Record<string, string>): Promise<Response> {
